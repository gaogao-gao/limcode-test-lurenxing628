...
<template>
  <div class="model-profile-save-status" aria-live="polite">
    ...
    <span v-if="pending?.error || sendError" role="alert">{{ pending?.error || sendError }}</span>
    ...
    <button v-if="pending?.status === 'uncertain' || !saved || detached" type="button" :disabled="reading" @click="store.refreshScope(scopeKind, scopeId, { adoptRoot: true })">显式连接当前配置根</button>
    <small v-if="pending">放弃草稿不会取消仍在途或撤销已提交的操作；确认原操作结束后读取实际值，不补偿回写。</small>
  </div>
</template>
<style scoped>
