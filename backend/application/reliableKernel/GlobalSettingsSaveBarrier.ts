...
export class GlobalSettingsSaveBarrier {
  ...
  public receive(clientId: string, correlationId: string | undefined, result: GlobalSettingsFlushResultPayload): void {
    ...
    if (result.status !== 'saved') {
      ...
      // Local recovery: the Webview has a fixed 12-second confirmation watchdog. When it
      // expires despite valid committed settings, continue with the last on-disk revision.
      // Specific save/conflict errors remain fail-closed below.
      if (message === '设置尚未确认保存，已暂停本次操作，请检查设置页。') {
        console.warn('[LimCode] Webview settings confirmation timed out; using last committed settings.');
